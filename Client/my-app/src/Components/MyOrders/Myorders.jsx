...
const QR_URL =
  "https://scontent.fsgn5-1.fna.fbcdn.net/v/t1.15752-9/566702663_1712219139468150_1685015265997950894_n.jpg";
...
function Myorders() {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const res = await axios.get(`${API_BASE_URL}/getOrdersByUser/${userId}`);
  ...
  const downloadInvoice = (order) => {
    ...
    doc.text(
      ...
      10,
      120 + (order.products?.length || 0) * 10
    );
  ...
  const cancelOrder = async (orderId) => {
    ...
    try {
      const res = await axios.put(`${API_BASE_URL}/cancelOrder/${orderId}`);
      if (res.data.success) {
        setOrders((prev) =>
          prev.map((order) =>
            order._id === orderId
